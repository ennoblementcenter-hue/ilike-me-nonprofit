...
function setPageTitle(path) {
  ...
  document.title = `${match ? match.title : "Page"} — I LIKE ME`;
}
...
const BRAND = {
  ...
  colors: { teal: "#00A9C6", orange: "#FF6A13", dark: "#0f172a" },
};
const Logo = ({ className = "h-9 w-auto" }) => (
  <img
    ...
    onError={(e) => {
      e.currentTarget.src =
        "https://dummyimage.com/200x200/00a9c6/ffffff&text=I+LIKE+ME";
    }}
  />
);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden md:flex gap-1">
            {nav.map(([to, label]) => (
              <button
                key={to}
                onClick={() => navigate(to)}
                className={`px-3 py-2 rounded-full text-sm font-medium hover:bg-slate-100 ${
                  path === to ? "bg-slate-100" : ""
                }`}
              >
                {label}
              </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              className="md:hidden inline-flex items-center rounded-full p-2 hover:bg-slate-100"
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid gap-8 md:grid-cols-4">
          ...
          <FooterCol title="Organization">
            <FooterLink navigate={navigate} to="/about">
              About
            </FooterLink>
            <FooterLink navigate={navigate} to="/pillars">
              Pillars
            </FooterLink>
            <FooterLink navigate={navigate} to="/programs">
              Programs
            </FooterLink>
            <FooterLink navigate={navigate} to="/impact">
              Impact
            </FooterLink>
          </FooterCol>
          <FooterCol title="Engage">
            <FooterLink navigate={navigate} to="/get-involved">
              Get Involved
            </FooterLink>
            <FooterLink navigate={navigate} to="/testimonials">
              Testimonials
            </FooterLink>
            <FooterLink navigate={navigate} to="/gallery">
              Gallery
            </FooterLink>
            <FooterLink navigate={navigate} to="/resources">
              Resources
            </FooterLink>
          </FooterCol>
          <FooterCol title="Legal">
            <FooterLink navigate={navigate} to="/privacy">
              Privacy
            </FooterLink>
            <FooterLink navigate={navigate} to="/terms">
              Terms
            </FooterLink>
          </FooterCol>
        </div>
        <div className="mt-8 flex items-center justify-between border-t border-slate-200 pt-6 text-sm text-slate-600">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              style={{ borderColor: BRAND.colors.teal, color: BRAND.colors.teal }}
            >
              Contact
            </button>
          </div>
...
function FooterLink({ to, children, navigate }) {
  return (
    <button onClick={() => navigate(to)} className="text-left hover:underline">
      {children}
    </button>
  );
}
...
function Hero({ navigate }) {
  return (
    <section
      ...
      style={{ background: `linear-gradient(135deg, ${BRAND.colors.teal}0D, #ffffff)` }}
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-24 grid lg:grid-cols-2 gap-10 items-center">
        <div>
          <div
            ...
            style={{ backgroundColor: `${BRAND.colors.teal}1A`, color: BRAND.colors.teal }}
          >
            Faith-forward • Evidence-informed • Community-powered
          </div>
          <h1
            className="mt-4 text-4xl sm:text-5xl font-black tracking-tight"
            style={{ color: BRAND.colors.dark }}
          >
            You are not broken. You are becoming.
          </h1>
          <p className="mt-4 text-lg text-slate-700 max-w-xl">
            We help youth and families heal shame, grow self-worth, and lead with courage through our Six Pillars:
            Self-Care, Self-Respect, Self-Esteem, Self-Acceptance, Self-Trust, and Self-Empowerment.
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            <button
              onClick={() => navigate("/programs")}
              className="rounded-full px-5 py-3 text-white font-semibold"
              style={{ backgroundColor: BRAND.colors.orange }}
            >
              Explore Programs
            </button>
          ...
          <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { k: "Youth served", v: "1,200+" },
              { k: "Workshops", v: "250+" },
              { k: "Partners", v: "35+" },
              { k: "Volunteer hrs", v: "8,000+" },
            ].map((m) => (
              <div key={m.k} className="rounded-2xl border p-4">
                <div className="text-sm text-slate-600">{m.k}</div>
                <div className="text-2xl font-black" style={{ color: BRAND.colors.teal }}>
                  {m.v}
                </div>
              </div>
            ))}
        ...
        <div className="relative">
          <div className="rounded-3xl overflow-hidden shadow-xl ring-1 ring-slate-200">
            <img
              ...
              onError={(e) => {
                e.currentTarget.src =
                  "https://images.unsplash.com/photo-1529336953121-ad5a0d43d0fa?auto=format&fit=crop&w=1200&q=60";
              }}
            />
          </div>
          <div
            className="absolute -bottom-6 -right-6 rounded-2xl px-4 py-3 text-white text-sm font-semibold"
...
function PillarsPreview({ navigate }) {
  return (
    <Section
      id="pillars-preview"
      title="Six Pillars of I LIKE ME"
      intro="The backbone of every curriculum, workshop, and mentorship touchpoint."
    >
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {pillars.map((p) => (
          <div key={p.title} className="rounded-2xl border p-6 bg-white">
            <h3 className="text-xl font-bold" style={{ color: BRAND.colors.teal }}>
              {p.title}
            </h3>
        ...
        {items.map((it) => (
          <div key={it.title} className="rounded-2xl border bg-white overflow-hidden">
            <img
              ...
              onError={(e) => {
                e.currentTarget.src =
                  "https://images.unsplash.com/photo-1517232115160-ff93364542dd?auto=format&fit=crop&w=1200&q=60";
              }}
            />
            <div className="p-5">
              <h3 className="text-lg font-bold" style={{ color: BRAND.colors.teal }}>
                {it.title}
              </h3>
              <p className="text-sm text-slate-600 mt-1">{it.body}</p>
              <button
                onClick={() => navigate("/programs")}
                className="mt-3 rounded-full px-4 py-2 text-white text-sm font-semibold"
                style={{ backgroundColor: BRAND.colors.orange }}
              >
                Learn more
              </button>
...
// ------- About / Pillars -------
const pillars = [
  { title: "Self-Care", body: "Sustainable rhythms for body, mind, and spirit. Rest, boundaries, recovery." },
  ...
  { title: "Self-Acceptance", body: "Own your narrative. Release shame. Integrate past pain into purpose." },
  { title: "Self-Trust", body: "Discern wisely. Keep promises to yourself. Build inner credibility." },
  { title: "Self-Empowerment", body: "Act with agency. Lead change in community. Choose courageous steps." },
];
function About() {
  return (
    <Section id="about" title="About I LIKE ME" intro="Trauma-informed, equity-centered, rooted in faith and community.">
      <div className="prose prose-slate max-w-none">
        <p>
          {BRAND.name} equips youth and the adults who serve them to heal shame and build durable self-worth. Our pedagogy
          blends research-backed SEL practices with spiritual formation and community action.
        </p>
        <h3>Mission</h3>
...
function Pillars() {
  return (
    <Section
      id="pillars"
      title="Six Pillars"
      intro="Each pillar includes age-banded curricula, caregiver guides, facilitator notes, and impact measures."
    >
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {pillars.map((p) => (
...
function programCards() {
  return [
    ...
    { title: "I LIKE ME Intimate Violence Prevention", body: "Trauma-informed care, self-respect, and healthy relationship dynamics.", img: "/images/ivp.png" },
    { title: "I LIKE ME LGBTQ Empowerment", body: "Affirming supports for LGBTQ youth—self-acceptance, esteem, resilience.", img: "/images/lgbtq.png" },
    { title: "I LIKE ME Staff & Administrator Curriculum", body: "Equip adults with empathy, language, and tools to reinforce Six Pillars.", img: "/images/staff.png" },
  ];
}
function Programs() {
  const items = programCards();
          ...
          <div key={it.title} className="rounded-2xl border bg-white overflow-hidden">
            ...
            <div className="p-5">
              ...
              <div className="mt-4 flex gap-2">
                <a href="#" className="rounded-full px-4 py-2 text-white text-sm font-semibold" style={{ backgroundColor: BRAND.colors.orange }}>
                  Book a call
                </a>
                <a href="#" className="rounded-full px-4 py-2 text-sm font-semibold border" style={{ borderColor: BRAND.colors.teal, color: BRAND.colors.teal }}>
                  Request proposal
                </a>
              </div>
            </div>
...
function DownloadCard({ title, desc, href }) {
  return (
    <div className="rounded-2xl border bg-white p-6 flex items-center justify-between">
      <div>
        <h3 className="font-semibold" style={{ color: BRAND.colors.teal }}>
          {title}
        </h3>
        <p className="text-sm text-slate-600">{desc}</p>
      </div>
      <a href={href} className="rounded-full px-4 py-2 text-white text-sm font-semibold" style={{ backgroundColor: BRAND.colors.orange }}>
        Download
      </a>
    </div>
  );
...
function Testimonials() {
  ...
  useEffect(() => {
    timer.current = setInterval(() => setI((p) => (p + 1) % items.length), 5000);
    return () => clearInterval(timer.current);
  }, []);
  return (
    <div className="rounded-3xl border bg-white p-6 md:p-10">
      <p className="text-lg md:text-xl font-semibold leading-snug">“{items[i].quote}”</p>
      ...
      <div className="mt-6 flex gap-2">
        {items.map((_, idx) => (
          <button
            ...
            onClick={() => setI(idx)}
            className={`h-2 w-2 rounded-full ${i === idx ? "opacity-100" : "opacity-40"}`}
            style={{ backgroundColor: BRAND.colors.teal }}
          />
        ))}
      </div>
...
function Card({ title, body }) {
  return (
    <div className="rounded-2xl border bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
      <h3 className="text-lg font-semibold" style={{ color: BRAND.colors.teal }}>
        {title}
      </h3>
  ...
  return (
    <section className="py-10 text-white" style={{ background: `linear-gradient(90deg, ${BRAND.colors.teal}, ${BRAND.colors.orange})` }}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <button onClick={() => navigate("/contact")} className="rounded-full bg-white text-slate-900 font-semibold px-5 py-3 hover:opacity-90">
            Book a meeting
          </button>
          <button onClick={() => navigate("/donate")} className="rounded-full bg-black/10 font-semibold px-5 py-3 hover:bg-black/20">
            Donate
          </button>
        </div>
      </div>
...
// ------- Impact / Resources / Donate / Contact -------
function Impact() {
  return (
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={m.k} className="rounded-2xl border bg-white p-6">
            <div className="text-sm text-slate-600">{m.k}</div>
            <div className="text-3xl font-black" style={{ color: BRAND.colors.teal }}>
              {m.v}
            </div>
          </div>
        ))}
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((it) => (
          <div key={it.title} className="rounded-2xl border bg-white p-6">
            ...
            <button className="mt-3 rounded-full px-4 py-2 text-white text-sm font-semibold" style={{ backgroundColor: BRAND.colors.orange }}>
              Download
            </button>
          </div>
        ))}
  ...
  return (
    <Section id="donate" title="Donate" intro="Your gift powers workshops, mentoring, and resources that change trajectories.">
      <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {tiers.map((t) => (
          <div key={t.amt} className="rounded-2xl border bg-white p-6 text-center">
            ...
            <a href="#" className="mt-4 inline-block rounded-full px-5 py-3 text-white font-semibold" style={{ backgroundColor: BRAND.colors.orange }}>
              Give {t.amt}
            </a>
          </div>
        ))}
      ...
      <div className="mt-8 rounded-2xl border bg-orange-50 p-6" style={{ borderColor: `${BRAND.colors.orange}66` }}>
        ...
        <p className="text-sm text-orange-900/80">Become a sustaining donor to fund consistent programming and rapid response.</p>
        <a href="#" className="mt-3 inline-block rounded-full px-5 py-3 text-white font-semibold" style={{ backgroundColor: BRAND.colors.orange }}>
          Give Monthly
        </a>
      </div>
    </Section>
...
function GetFieldId(label) {
  return label.toLowerCase().replace(/[^a-z0-9]+/g, "-") + "-" + Math.random().toString(36).slice(2, 7);
}
function Contact() {
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <form onSubmit={submit} className="rounded-2xl border bg-white p-6">
          <div className="grid gap-3">
            ...
            <button className="rounded-full px-5 py-3 text-white font-semibold" style={{ backgroundColor: BRAND.colors.orange }}>
              Send
            </button>
          </div>
        </form>
        <div className="rounded-2xl border bg-white p-6">
          ...
          <h3 className="mt-4 text-lg font-semibold" style={{ color: BRAND.colors.teal }}>
            Email
          </h3>
          <a className="text-sm" style={{ color: BRAND.colors.teal }} href="mailto:info@ilike.me">
            info@ilike.me
          </a>
          <h3 className="mt-4 text-lg font-semibold" style={{ color: BRAND.colors.teal }}>
            Social
  ...
  return (
    <label className="grid gap-1">
      ...
      <input
        ...
        style={{ borderColor: `${BRAND.colors.teal}66` }}
      />
    </label>
  );
}
...
function SignupForm() {
  return (
    <div className="rounded-2xl border bg-white p-6">
      <h3 className="font-semibold" style={{ color: BRAND.colors.teal }}>Volunteer signup</h3>
      <p className="text-sm text-slate-600">We’ll follow up with orientation dates.</p>
      <div className="grid gap-3 mt-3">
        <Input label="Full name" />
        <Input label="Email" type="email" />
        <Select label="Interest" value={"Mentor"} onChange={() => {}} options={["Mentor", "Facilitator", "Events"]} />
        <button className="rounded-full px-5 py-3 text-white font-semibold" style={{ backgroundColor: BRAND.colors.orange }}>
          Submit
        </button>
      </div>
...
function PartnerForm() {
  return (
    <div className="rounded-2xl border bg-white p-6">
      <h3 className="font-semibold" style={{ color: BRAND.colors.teal }}>Partner inquiry</h3>
      <p className="text-sm text-slate-600">Schools, churches, nonprofits—let’s co-design.</p>
      <div className="grid gap-3 mt-3">
        <Input label="Organization" />
        <Input label="Contact email" type="email" />
        <Select label="Program of interest" value={"Youth Program"} onChange={() => {}} options={["Youth Program","Bedside Intervention","Intimate Violence Prevention","LGBTQ Empowerment","Staff & Admin"]} />
        <button className="rounded-full px-5 py-3 text-white font-semibold" style={{ backgroundColor: BRAND.colors.orange }}>
          Request meeting
        </button>
      </div>
...
function Router({ path, navigate }) {
  ...
  switch (path) {
    ...
    default:
      return (
        <Section id="404" title="Not found">
...
function Privacy() {
  return (
    <Section id="privacy" title="Privacy Policy" intro="Your privacy matters.">
      <div className="prose prose-slate max-w-none">
        <p>We collect minimal data to deliver services, newsletters, and donation receipts. We do not sell your data.</p>
